...
# HIGH PRIORITY:
- Build the 2-D cost-analysis pipeline `validation/cost_analysis_free_swimming_2d/` (copy + adapt of `run_cost_analysis.py`, `run_multigrid_cost_analysis.py`, `run_scaling_conditions_pipeline.py`, `plot_scaling.py`) targeting a 2-D scene exercising the new `streaming_sdf_2d` / `streaming_forces_2d` path.  Kernels and solver wiring are already in place (see DONE).
