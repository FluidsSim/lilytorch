...
(hx, hy, hz, dx, dy, dz) instead of relying on ``self``.  Spacing
arguments (hx, hy, hz) may be either scalar floats or tensors of the
same shape as the field being differentiated; the latter enables
variable per-cell spacing for adaptive / non-uniform grids.
"""
...
# Gradient
# ------------------------------------------------------------------
def gradient(var, hx, hy, ndim, hz=None):
    """
    Compute gradient(var) using per-cell spacing tensors.

    Parameters
    ----------
    var  : field tensor
    hx   : scalar or tensor — grid spacing in x  (shape (nx,ny) or (nx,ny,nz))
    hy   : scalar or tensor — grid spacing in y
    ndim : 2 or 3
    hz   : scalar or tensor — grid spacing in z  (3-D only)
    ...
    if ndim == 2:
        dvar_dx[1:-1, 1:-1] = (var[1:-1, 1:-1] - var[:-2, 1:-1]) / hx[1:-1, 1:-1]
        dvar_dy[1:-1, 1:-1] = (var[1:-1, 1:-1] - var[1:-1, :-2]) / hy[1:-1, 1:-1]
        return (dvar_dx, dvar_dy)
    else:
        dvar_dz = torch.zeros_like(var)
        dvar_dx[1:-1, 1:-1, 1:-1] = (var[1:-1, 1:-1, 1:-1] - var[:-2, 1:-1, 1:-1]) / hx[1:-1, 1:-1, 1:-1]
        dvar_dy[1:-1, 1:-1, 1:-1] = (var[1:-1, 1:-1, 1:-1] - var[1:-1, :-2, 1:-1]) / hy[1:-1, 1:-1, 1:-1]
        dvar_dz[1:-1, 1:-1, 1:-1] = (var[1:-1, 1:-1, 1:-1] - var[1:-1, 1:-1, :-2]) / hz[1:-1, 1:-1, 1:-1]
...
# Vorticity
# ------------------------------------------------------------------
def vorticity(u, v, hx, hy, ndim, w=None, hz=None):
    """
    Compute vorticity using per-cell spacing tensors.

    Parameters
    ----------
    hx, hy : scalar or tensor — per-cell spacing in x and y
    hz     : scalar or tensor — per-cell spacing in z (3-D only)
    ...
    if ndim == 2 or w is None:
        ...
        dvdx[1:-1, 1:-1] = (v[1:-1, 1:-1] - v[:-2, 1:-1]) / hx[1:-1, 1:-1]
        dudy[1:-1, 1:-1] = (u[1:-1, 1:-1] - u[1:-1, :-2]) / hy[1:-1, 1:-1]
        return dvdx - dudy
    else:
        ...
        ox[2:-2, 2:-2, 2:-2] = (
            (w[2:-2, 2:-2, 2:-2] - w[2:-2, 1:-3, 2:-2]) / _hy -
            (v[2:-2, 2:-2, 2:-2] - v[2:-2, 2:-2, 1:-3]) / _hz
        )
        oy = torch.zeros_like(u)
        oy[2:-2, 2:-2, 2:-2] = (
            (u[2:-2, 2:-2, 2:-2] - u[2:-2, 2:-2, 1:-3]) / _hz -
            (w[2:-2, 2:-2, 2:-2] - w[1:-3, 2:-2, 2:-2]) / _hx
        )
        oz = torch.zeros_like(u)
        oz[2:-2, 2:-2, 2:-2] = (
            (v[2:-2, 2:-2, 2:-2] - v[1:-3, 2:-2, 2:-2]) / _hx -
            (u[2:-2, 2:-2, 2:-2] - u[2:-2, 1:-3, 2:-2]) / _hy
        )
        return torch.sqrt(ox**2 + oy**2 + oz**2)


def vorticity_components(u, v, w, hx, hy, hz):
    """
    ...
    Parameters
    ----------
    hx, hy, hz : scalar or tensor — per-cell spacing fields
    """
    _hx = hx[2:-2, 2:-2, 2:-2]
    _hy = hy[2:-2, 2:-2, 2:-2]
    _hz = hz[2:-2, 2:-2, 2:-2]
