...
class FluidSolver:
    ...
    def gradient(self, var):
        return ops.gradient(var, self.hx, self.hy, self.ndim, hz=getattr(self, 'hz', None))
    ...
    def vorticity(self, u, v, w=None):
        return ops.vorticity(u, v, self.hx, self.hy, self.ndim, w=w,
                             hz=getattr(self, 'hz', None))
    def vorticity_components(self, u, v, w):
        return ops.vorticity_components(u, v, w, self.hx, self.hy, self.hz)
    ...
    def __init__(self, pars, dtype=torch.float32, costum_update=None, compute_forces=True):
        ...
        if abs(float(self.dx-self.dy)) >= 1e-10:
            warnings.warn(
                "Grid spacing in x = {} and y = {} are not equal. "
                "Non-square cells are supported via hx/hy tensor fields "
                "but may affect some operators that rely on scalar self.h.".format(
                    self.dx, self.dy),
                UserWarning,
            )
        self.h = self.dx
        ...
        if "Nz" in solver:
            ...
            if abs(float(self.dx - self.dz)) >= 1e-10:
                warnings.warn(
                    "Grid spacing in x = {} and z = {} are not equal.".format(
                        self.dx, self.dz),
                    UserWarning,
                )
        ...
        # ---- Per-cell spacing fields (uniform on a regular grid) ----
        # hx[i,j] = grid spacing in x at cell (i,j), shape (nx, ny)
        # hy[i,j] = grid spacing in y at cell (i,j), shape (nx, ny)
        # Use set_spacing(hx, hy) / set_spacing(hx, hy, hz) to inject
        # non-uniform spacing for adaptive / nested grids.
        self.hx = self.dx * torch.ones((self.nx, self.ny), device=self.device, dtype=self.dtype)
        self.hy = self.dy * torch.ones((self.nx, self.ny), device=self.device, dtype=self.dtype)
        if self.ndim == 3:
            self.hz = self.dz * torch.ones((self.nx, self.ny, self.nz),
                                           device=self.device, dtype=self.dtype)
