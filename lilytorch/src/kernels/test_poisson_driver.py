...
from lilytorch.src.poisson_mult import PoissonSolver as WarpPoisson
...
def test_warp_poisson_graphed_multigrid_2d(dtype, smoother):
    """The CUDA-graphed all-Warp 2-D multigrid (``WarpMG2D``) converges to the same
    residual as the native Python-path solver — now dtype-generic (f32+f64) and
    rbgs/jacobi, matching the 3-D driver (no python-loop fallback for any config)."""
    h, f, p, faces = _problem(2, dtype, N=64)
    rn = _solve(NativePoisson, "solve_multigrid", 2, dtype, smoother)
    s = WarpPoisson(dtype=dtype, device=DEV, h=h, tol=1e-6, max_vcycles=15,
                    nsmoothing=2, smoother=smoother, verbose=False,
                    use_kernels=True, cuda_graph=True)
    _, r = s.solve_multigrid(f.clone(), p, **{k: v.clone() for k, v in faces.items()})
