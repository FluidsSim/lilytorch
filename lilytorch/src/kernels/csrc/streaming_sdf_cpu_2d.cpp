...
//  two sequential passes over the per-body AABB.  Phase D uses the
//  CURRENT union-SDF normals from ``sdf_cc`` and the CURRENT per-body
//  CC-SDF support cached in the local sparse buffer, matching the
//  separate streaming+kernels force path.
// =====================================================================
    ...
    at::Tensor out)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(F_flat.scalar_type(), "streaming_sdf_forces_fused_2d_multi_cpu", [&] {
        ...
        // Phase C: finish the union/decode update for all bodies first so
        // the later force pass sees the current union SDF everywhere.
        for (int b = 0; b < B; ++b) {
            const int Ai = (int)dim_[b*2 + 0];
            const int Aj = (int)dim_[b*2 + 1];
            const int vol = Ai * Aj;
        ...
        // Phase D: resample each body's current cc-SDF support against the
        // fully updated union SDF and accumulate forces/torques.
        for (int b = 0; b < B; ++b) {
            const int Ai = (int)dim_[b*2 + 0];
            const int Aj = (int)dim_[b*2 + 1];
            const int vol = Ai * Aj;
