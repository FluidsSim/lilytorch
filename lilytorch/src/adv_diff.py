...
class AdvDiffSolver:
    ...
    * ``'semi-lagrangian'`` / ``'implicit'``   -- Stam 1999
    ...
    def __init__(
        ...
        # ---- Per-cell spacing fields (uniform by default) -------------
        # _DH[d] has shape (n[0], n[1]) in 2-D or (n[0], n[1], n[2]) in 3-D.
        # Call set_spacing(DX, DY) or set_spacing(DX, DY, DZ) to inject
        # non-uniform spacing for adaptive / nested grids.
        self._DH = [
            float(self.dh[d]) * torch.ones(tuple(self.n), device=device, dtype=x.dtype)
            for d in range(len(self.n))
        ]
    ...
    # -----------------------------------------------------------------
    # Per-cell spacing injection  (for adaptive / non-uniform grids)
    # -----------------------------------------------------------------
    def set_spacing(self, *DH: torch.Tensor):
        """Override uniform grid spacing with per-cell spacing tensors.

        Parameters
        ----------
        *DH : tensors — DX, DY (2-D) or DX, DY, DZ (3-D).
              Each must have shape ``(n[0], n[1])`` or
              ``(n[0], n[1], n[2])`` and reside on ``self.device``.

        After calling this method, the advection-diffusion solve will
        use the per-cell spacing for both the diffusion Laplacian and
        the convective flux divergence.
        """
        if len(DH) != self.ndim:
            raise ValueError(
                f"set_spacing expects {self.ndim} tensors, got {len(DH)}."
            )
        shape = tuple(self.n)
        for d, tensor in enumerate(DH):
            if tensor.shape != shape:
                raise ValueError(
                    f"Spacing tensor for dim {d} has shape {tensor.shape}, "
                    f"expected {shape}."
                )
            self._DH[d] = tensor
        # Update legacy scalar helpers (use mean for backward-compat APIs)
        self.dh       = [float(dh.mean()) for dh in self._DH]
        self._inv_dh2 = [1.0 / (h * h) for h in self.dh]
        ...
        if self.ndim == 3:
            self.DZ = self._DH[2]
        # Update legacy per-axis scalars
        self.dx, self.dy = self.dh[0], self.dh[1]
        self.dtdx, self.dtdy = self._dt_dh[0], self._dt_dh[1]
        self.dtdx2 = self.dtdx / self.dh[0]
        self.dtdy2 = self.dtdy / self.dh[1]
    ...
    def _laplacian(phi, inv_dh2):
        ...
        ----------
        phi      : field tensor
        inv_dh2  : list of scalars or list of tensors (inner-cell shape).
                   When tensors are provided (non-uniform spacing), each
                   entry ``inv_dh2[d]`` must have the shape of the interior
                   slice ``phi[1:-1, 1:-1, ...]``.
        """
        ndim  = phi.ndim
        inner = _inner(ndim)
    ...
    def _solve_convective(self, *vel, iteration=0):
        ...
        Uses per-cell spacing tensors (self._DH) so the step is correct
        on both uniform and non-uniform / adaptive grids.
        """
        ndim    = self.ndim
        vel_new = [v.clone() for v in vel]
        inner   = _inner(ndim)

        # Per-cell inverse spacing squared (for Laplacian) and dt/DH (for advection)
        inv_dh2    = [1.0 / (self._DH[d][inner] ** 2) for d in range(ndim)]
        dt_over_dh = [self.dt / self._DH[d][inner] for d in range(ndim)]

        for i in range(ndim):
            # diffusion
            rhs = self.nu * self.dt * self._laplacian(vel[i], inv_dh2)
            # convective fluxes in each direction
            for d in range(ndim):
                fv = self._face_vel(vel, i, d)
                p  = self._field_for_flux(vel[i], d, ndim)
                F  = self._flux(fv, p, d)
                rhs = rhs + dt_over_dh[d] * (
