...
try:
    ...
    RegularGridInterpolatorAutomatic = None
...
class AdvDiffSolver:
    ...
    * ``'semi-lagrangian'`` / ``'implicit'``   -- Stam 1999
    ...
    def __init__(
        ...
        sgs_model : str or None -- sub-grid scale / stabilisation model:
                      None                  – no extra diffusion (default)
                      "smagorinsky"         – classic Smagorinsky SGS model
                                              (ν_t = (Cs·Δ)²·|S̄|)
                      "artificial_diffusion" – SUPG-inspired local diffusion
                                              (ν_art = Cv·h·|u|)
        Cs    : float -- Smagorinsky constant (default 0.17, range 0.1–0.2)
        Cv    : float -- artificial-diffusion coefficient (default 0.1)
        """
        self.device = device
        self.dtype  = x.dtype
        self.dt     = dt
        ...
        # LES filter width: geometric mean of mesh spacings
        _delta_prod = 1.0
        for _h in self.dh:
            _delta_prod *= _h
        self.delta_sgs = float(_delta_prod ** (1.0 / self.ndim))
        ...
        if method in _schemes:
            ...
            if RegularGridInterpolatorAutomatic is None:
                raise ImportError(
                    "The 'pytorch_interpolation' package is required for "
                    "method='semi-lagrangian'/'implicit'. "
        ...
        print(f"Using the {method} method for the adv-diff equation ({self.ndim}D, sgs_model={sgs_model})")

    # =================================================================
    # Sub-grid scale / stabilisation models
    # =================================================================

    def smagorinsky_viscosity(self, *vel):
        """Classic Smagorinsky (1963) SGS eddy-viscosity model.

        Returns a tensor of interior-cell SGS viscosity:

            ν_t = (Cs · Δ)² · |S̄|

        where
            Δ   = (Π dh_i)^{1/ndim}     LES filter width (geometric mean)
            |S̄| = √(2 · Sij · Sij)      strain-rate magnitude
            Sij = ½(∂u_i/∂x_j + ∂u_j/∂x_i)

        Velocity gradients use 2nd-order centred differences on interior cells.
        Works in 2-D and 3-D.
        """
        ndim  = self.ndim
        inner = _inner(ndim)

        S_sq = torch.zeros_like(vel[0][inner])
        for i in range(ndim):
            for j in range(ndim):
                # ∂vel[i]/∂x_j at interior cells
                fwd_j = list(inner); fwd_j[j] = slice(2, None)
                bwd_j = list(inner); bwd_j[j] = slice(None, -2)
                dvi_dxj = (
                    vel[i][tuple(fwd_j)] - vel[i][tuple(bwd_j)]
                ) / (2.0 * self.dh[j])

                # ∂vel[j]/∂x_i at interior cells
                fwd_i = list(inner); fwd_i[i] = slice(2, None)
                bwd_i = list(inner); bwd_i[i] = slice(None, -2)
                dvj_dxi = (
                    vel[j][tuple(fwd_i)] - vel[j][tuple(bwd_i)]
                ) / (2.0 * self.dh[i])

                Sij = 0.5 * (dvi_dxj + dvj_dxi)
                S_sq += Sij ** 2  # sum_{i,j} Sij^2 over all pairs

        # |S̄|² = 2 * Sij * Sij  (Einstein summation, i.e. sum over all i,j)
        S_mag = torch.sqrt(torch.clamp(2.0 * S_sq, min=0.0))
        return (self.Cs * self.delta_sgs) ** 2 * S_mag

    def artificial_diffusion_viscosity(self, *vel):
        """SUPG-inspired local artificial diffusion.

        Returns interior-cell artificial viscosity:

            ν_art = Cv · h · |u_local|

        where
            h         = (Π dh_i)^{1/ndim}   mesh size (geometric mean)
            |u_local| = √(Σ u_i²)            local velocity magnitude

        Works in 2-D and 3-D.
        """
        inner = _inner(self.ndim)
        u_mag = torch.sqrt(sum(v[inner] ** 2 for v in vel))
        return self.Cv * self.delta_sgs * u_mag

    def compute_nu_sgs(self, *vel):
        """Return SGS viscosity at interior cells, or scalar 0.0 when disabled."""
        if self.sgs_model == "smagorinsky":
            return self.smagorinsky_viscosity(*vel)
        elif self.sgs_model == "artificial_diffusion":
            return self.artificial_diffusion_viscosity(*vel)
        return 0.0

    def _nu_t_full(self, *vel):
        """Full-field SGS viscosity tensor (zeros at ghost cells).

        Returns ``None`` when no SGS model is active, so callers can use
        ``if nu_t is None`` to switch between constant- and variable-
        coefficient diffusion operators.
        """
        nu_inner = self.compute_nu_sgs(*vel)
        if isinstance(nu_inner, float):   # scalar 0.0 → no model
            return None
        nu_t = torch.zeros_like(vel[0])
        nu_t[_inner(self.ndim)] = nu_inner
        return nu_t
    ...
    def _solve_convective(self, *vel, nu_t=None, iteration=0):
        ...
        When *nu_t* is ``None`` and ``self.sgs_model`` is set, SGS viscosity
        is computed automatically.  When *nu_t* is provided explicitly it
        takes precedence (useful for external LES couplings).

        When effective viscosity is constant:
            diff = nu * lap(phi)
        When it is spatially variable (Smagorinsky or artificial diffusion):
            diff = div((nu + nu_t) * grad(phi))   [variable-coeff Laplacian]

        Accepts (u, v) in 2-D or (u, v, w) in 3-D.
        """
        if nu_t is None and self.sgs_model is not None:
            nu_t = self._nu_t_full(*vel)
        ndim    = self.ndim
