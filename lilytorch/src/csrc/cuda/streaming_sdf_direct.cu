// =====================================================================
//  streaming_sdf_direct.cu — Regime-A direct-write kernels (2-D and 3-D)
// =====================================================================
...
// =====================================================================
//  2-D direct kernel — one grid over union dirty AABB
// =====================================================================
template <typename scalar_t>
    ...
    scalar_t* __restrict__ body_u, scalar_t* __restrict__ body_v, int interp)
{
    ...
    scalar_t best_cc = sdf_cc[g_idx];
    scalar_t best_u  = sdf_u[g_idx];
    scalar_t best_v  = sdf_v[g_idx];
    scalar_t bu = body_u[g_idx], bv = body_v[g_idx];
    ...
    int64_t dirty_i0, int64_t dirty_j0, int64_t dirty_Ai, int64_t dirty_Aj)
{
    ...
    int64_t vol = dirty_Ai * dirty_Aj;
    int bs = (vol <= 256) ? 32 : (vol <= 65536) ? 128 : 256;
...
// =====================================================================
//  3-D direct kernel — one grid over union dirty AABB
// =====================================================================
template <typename scalar_t>
__global__ void streaming_sdf_stag_3d_direct_kernel(
    ...
    int Ngy, int Ngz, scalar_t half_h, int B,
    ...
    scalar_t* __restrict__ body_u, scalar_t* __restrict__ body_v, scalar_t* __restrict__ body_w, int interp)
{
    ...
    scalar_t best_u  = sdf_u[g_idx];
    scalar_t best_v  = sdf_v[g_idx];
    scalar_t best_w  = sdf_w[g_idx];
    scalar_t bu = body_u[g_idx], bv = body_v[g_idx], bw = body_w[g_idx];
    ...
    int64_t dirty_Ai, int64_t dirty_Aj, int64_t dirty_Ak)
{
    ...
    int64_t vol = dirty_Ai * dirty_Aj * dirty_Ak;
    int bs = (vol <= 256) ? 32 : (vol <= 65536) ? 128 : 256;
