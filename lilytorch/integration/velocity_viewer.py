"""
VelocityViewer – draw the linear (and, optionally, angular) velocity of each
immersed body as an arrow inside the MuJoCo viewer **and** in the recorded
video.

One arrow is drawn per FARMS link/body, anchored at the body centre of mass
(``data.xipos``) and pointing along the body's world-frame linear velocity.
When ``show_angular`` is enabled, a second arrow per body is drawn along the
angular-velocity axis (right-hand rule), length proportional to |ω|.

Appearance knobs:

- ``vel_scale``    – linear-arrow **length** per unit speed (metres per m/s).
- ``vel_width``    – the **circular (shaft) radius** of the linear arrow (m).
- ``show_angular`` – also draw an angular-velocity arrow per body (default
                     ``False``).
- ``ang_scale``    – angular-arrow length per unit |ω| (m per rad/s); defaults
                     to ``vel_scale``.
...
dependency on the FluidExtension)::
    ...
    extensions.append({
        ...
        "config": {
            "vel_scale"   : 0.1,    # metres of arrow per (m/s)
            "vel_width"   : 0.003,  # shaft (circular) radius in metres
            "color"       : "#00FF66",
            "max_length"  : 0.3,    # clamp arrow length (m); null = no clamp
            "min_vel"     : 0.0,    # hide arrows below this speed (m/s)
            "show_angular": True,   # also draw angular-velocity arrows
            "ang_scale"   : 0.05,   # m per rad/s (default: vel_scale)
            "ang_width"   : 0.002,  # shaft radius (default: vel_width)
            "ang_color"   : "#FFAA00",
...
def _parse_color(color, default=(0.0, 1.0, 0.4, 0.9)):
...
class VelocityViewer(TaskExtension):
    """Render the linear (and optionally angular) velocity of each body."""

    def __init__(
        self,
        experiment_options: ExperimentOptions,
        vel_scale: float = 0.1,
        vel_width: float = 0.003,
        color="#00FF66",
        ...
        self.vel_width = float(vel_width)
        ...
        self.ang_scale = (self.vel_scale if ang_scale is None
                          else float(ang_scale))
        self.ang_width = (self.vel_width if ang_width is None
                          else float(ang_width))
        self.ang_color = _parse_color(ang_color,
                                      default=(1.0, 0.67, 0.0, 0.9))
        ...
        self._sv = np.zeros(6, dtype=np.float64)   # scratch 6D spatial velocity
    ...
    def from_options(cls, config: dict, experiment_options: ExperimentOptions):
        return cls(
            ...
            vel_scale=config.get("vel_scale", 0.1),
            vel_width=config.get("vel_width", 0.003),
            color=config.get("color", "#00FF66"),
            max_length=config.get("max_length", None),
            min_vel=config.get("min_vel", 0.0),
            show_angular=config.get("show_angular", False),
            ang_scale=config.get("ang_scale", None),
            ang_width=config.get("ang_width", None),
            ang_color=config.get("ang_color", "#FFAA00"),
    ...
    def _reserve_slots(self, n_bodies: int):
        ...
        self._width = np.full(self._max_arrows, self.vel_width, dtype=np.float64)
        ...
        if self._viewer is not None:
            ...
            if reserved < self._max_arrows:
                print(f"[VelocityViewer] WARNING: user_scn.maxgeom={scn.maxgeom} "
                      f"reached – only {reserved}/{self._max_arrows} arrow "
                      f"slots reserved.")
                self._max_arrows = reserved
            print(f"[VelocityViewer] Reserved {self._max_arrows} arrow slots "
                  f"(geom {self._geom_start}..{scn.ngeom - 1}); "
                  f"{self._arrows_per_body} per body.")
        else:
            print("[VelocityViewer] No viewer (headless) – "
    ...
    def _patch_camera_renderers(self, task: ExperimentTask):
        ...
        for ext in task.extensions:
            ...
            print(f"[VelocityViewer] Patched {type(ext).__name__} renderer "
    ...
    def before_step(self, task: ExperimentTask, action, physics: Physics):
        ...
        # Read per-body spatial velocity from MuJoCo (world frame, at COM).
        ...
        for body_i in range(min(self._n_bodies, len(self._body_mj_ids))):
            ...
            mujoco.mj_objectVelocity(
                model_ptr, data_ptr, _OBJ_BODY, ind, sv, 0,  # 0 = world frame
            )
            ang = sv[:3]
            lin = sv[3:]
            ...
            n_active = self._push_arrow(
                n_active, base, lin,
                self.vel_scale, self.min_vel, self.color, self.vel_width,
            )
