    ...
    def __init__(self, animat_data, animat_options, experiment_options, config, animat_i, *args, **kwargs):
        ...
        with open('muscle_parameters/muscle_parameters_FN_5000_ZC_750_G0_419.csv', newline='') as csvfile:
            ...
            for row in reader:
                ...
                if joint_i==12:
                    mult=2
                    sqrt_mult=np.sqrt(mult)
                elif joint_i==13:
                    mult=5
                    sqrt_mult=np.sqrt(mult)
                elif joint_i==14 or joint_i==0:
                    mult=15
                    sqrt_mult=np.sqrt(mult)
                else:
                    mult=1
                ...
                muscles_params[-1]["alpha"] *= mult
                muscles_params[-1]["beta"]  *= mult
                muscles_params[-1]["delta"] *= sqrt_mult
