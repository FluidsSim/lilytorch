...
class SimConfig(BaseSimConfig):
    ...
    def __init__(self):
        ...
        drag_file = os.path.join(os.path.dirname(__file__), "drag.npy")
        loaded_drags = np.asarray(np.load(drag_file), dtype=float)
        self.constant_drags = [[row[:3].tolist(), row[3:].tolist()] for row in loaded_drags]
