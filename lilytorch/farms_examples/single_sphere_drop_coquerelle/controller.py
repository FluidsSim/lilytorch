...
class BDIMhandler():
    ...
    def __init__(self, yaml_file, data, physics, dtype=torch.float64):
        ...
        # initialize solref
        physics.model.geom_solref[:,0]= 0.00001
        physics.model.geom_solref[:,1]= 10
    ...
    def apply_forces(self, task, physics):
        ...
        for body_i, body in enumerate(self.fluid_solver.composite_body.bodies[:]):
            ...
            # g = -980
            # torque =(self.friction_force_ang_z[body_i] + self.pressure_force_ang_z[body_i])
            ...
            # print("Friction F_z: {}, Pressure P_z: {}, qvel: {}".format(fz/mass, self.pressure_force_y[0]/mass, physics.data.qvel))
            # print(physics.data.xfrc_applied[ind_task, [0,2,4]])
            print(physics.data.xfrc_applied[ind_task]/mass, physics.data.qvel)
    ...
    def step(self, task, physics):
        ...
        if not self.terminate:
            ...
            self.fluid_solver.rho = (1.0*self.fluid_solver.mu0_all_u+1.5*self.fluid_solver.m_m0_all_u)

            v-=980*self.fluid_solver.dt
            ...
            # coeff = timestep/self.fluid_solver.rho
            # p = self.fluid_solver.poisson_solverPETSc.solve(-self.fluid_solver.div/coeff).reshape(self.fluid_solver.nx,self.fluid_solver.ny)
            # (p_x, p_y) = self.fluid_solver.gradient(p)
            # (u,v)=(uprime-coeff*p_x, vprime-coeff*p_y)
