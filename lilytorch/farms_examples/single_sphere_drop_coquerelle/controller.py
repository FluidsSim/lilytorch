"""Network controller – 2-D sphere sedimentation (Coquerelle et al.)

Updated to be compatible with the current lilytorch API (MultiAnimatBodies,
BDIM2, forces_method2 per-body SDF fields, etc.).
"""
...
class BDIMhandler:
    ...
    def apply_forces(self, task, physics):
        ...
        for body_i in range(len(fs.composite_body.bodies)):
            ...
            print(physics.data.xfrc_applied[ind] / mass, physics.data.qvel)
    ...
    def step(self, task, physics):
        ...
        if not self.terminate:
            ...
            fs.rho = 1.0 * fs.mu0_all_u + 1.5 * fs.m_m0_all_u

            (u, v, p) = (fs.u0, fs.v0, fs.p0)

            # 4. Gravity body force  (g = -980 in the y-direction)
            v = v - 980 * fs.dt
            ...
            # 8. Poisson solve (variable-density multigrid)
            c     = torch.ones_like(u)
