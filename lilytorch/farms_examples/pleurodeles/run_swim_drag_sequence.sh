...
# Run 6 drag simulations sequentially with different swim kinematics CSVs.
# Usage:
#   bash lilytorch/farms_examples/pleurodeles/run_swim_drag_sequence.sh

SCRIPT="gen_configs_swim_3d_drag.py"
CONFIG_FILE="gen_configs_swim_3d_drag.py"
...
for csv in "${CSV_LIST[@]}"; do
  ...
  # Patch the custom step count before each drag run.
  sed -E -i "s/^([[:space:]]*self\.n_iterations[[:space:]]*=[[:space:]]*).*/\1${n_iter}/" "${CONFIG_FILE}"

  echo "==============================================="
  echo "Running DRAG with LILYTORCH_SWIM_CSV=${csv} and n_iterations=${n_iter}"
  echo "==============================================="
  ...
  target_dir="${OUTPUT_ROOT}/Freq${freq}_adrag"
...
echo "All sequential drag runs completed."
