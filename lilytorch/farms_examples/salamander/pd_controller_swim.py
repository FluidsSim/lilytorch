...
class PositionController(KinematicsController):
    ...
    def generate_positions(
        ...
        limb_angles1 = limb_pose1*np.ones_like(times)
        limb_angles2 = limb_pose2*np.ones_like(times)

        limb_angles = np.zeros((times.shape[0], 8))

        limb_angles[:, range(0, 8, 2)] = limb_angles1
        limb_angles[:, range(1, 8, 2)] = limb_angles2
