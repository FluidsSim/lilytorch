...
class BDIMhandler():
    ...
    def update(self, t, iteration, dt=1):
        ...
        for data in self.data:
            ...
            com_poses = self.cython2numpy( data.sensors.links.com_positions()[iteration] ) [:, self.lin_axes]
            urdf_poses = self.cython2numpy( data.sensors.links.urdf_positions()[iteration] ) [:, self.lin_axes]
            r = self.cython2numpy(Rotation.from_quat(data.sensors.links.urdf_orientations()[iteration]).as_matrix().astype(self.dtype_np))[:, self.lin_axes, :][:, :, self.lin_axes]

            lin_vels = self.cython2numpy( data.sensors.links.com_lin_velocities()[iteration] ) [:, self.lin_axes]
            ang_vels = self.cython2numpy( [data.sensors.links.com_ang_velocity(iteration, link) for link in range(len(data.sensors.links.names))] )[:,self.ang_axes]

            for i, body in enumerate(self.fluid_solver.composite_body.bodies[:]):
                id=i # id of the body in FARMS
                ...
                self.fluid_solver.composite_body.sdf_vals_u[i]=body.sdf(newpos_u, newpos_v)
                ...
                self.fluid_solver.composite_body.sdf_vals_v[i]=body.sdf(newpos_u, newpos_v)
    ...
    def apply_forces(self, task, physics):
        ...
        # for animat_i, map in enumerate(task.maps):

        #     indices = map['sensors']['data2xfrc']

        #     from IPython import embed; embed()

        #     physics.data.xfrc_applied[indices, 2] = 9.81 * task.units.newtons
        ...
        #     map = task.maps[animat_i]

        # indices = [map['sensors']['data2xfrc'] for map in task.maps]
        # from IPython import embed; embed()
