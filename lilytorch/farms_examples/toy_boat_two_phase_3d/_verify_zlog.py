...
class BoatZLogger(TaskExtension):
    ...
    def before_step(self, task, action, physics):
        ...
        self._rows.append((self._it, t, z, pitch))
        if self._it % self.print_every == 0 or self._it < 5:
            print(f"[zlog] it={self._it:5d} t={t:.3f}s z={z:.4f} "
                  f"pitch={pitch:+.2f}deg Fz={fz:+.1f}N Ty={ty:+.1f}Nm", flush=True)
        self._it += 1

    def end_episode(self, task, physics):
        with open(self.log_path, "w") as f:
            f.write("iteration,time,z,pitch_deg\n")
            for it, t, z, p in self._rows:
                f.write(f"{it},{t:.6f},{z:.6f},{p:.6f}\n")
