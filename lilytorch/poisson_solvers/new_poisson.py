...
class PoissonSolver:
    ...
    def build_operators(self, c, h2):
        ...
        J_el = torch.zeros_like(c, device=self.device) # diagonal Jacobi elements
        J_el[1:-1,1:-1] = J_diag
        # J_el[0,:]       = 2*c[0,:]
        # J_el[-1,:]      = 2*c[-1,:]
        # J_el[:,0]       = 2*c[:,0]
        ...
        def LU(u):
            res=torch.zeros_like(u, device=self.device)
    ...
    def solve_multigrid(self, f, u, c, h2, m1=20, tol=1e-4, max_cycles=7):
        ...
        while r_err>tol and cycle<max_cycles:
            ...
            if self.verbose:
                print("Cycle number = {} - residual = {}".format(cycle, r_err_new))
            # if r_err_new>=r_err:
            #     if self.verbose:
    ...
    def multigrid(self, f, u, c, h2, r=None, m1=3):
        """
